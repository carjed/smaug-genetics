...
grep -Fwf  <(grep "^$1" /net/bipolar/jedidiah/mutation/reference_data/GoNL_DNMs2.txt | cut -f 2)  /net/bipolar/jedidiah/mutation/output/predicted/full/chr${chr}_full.txt | awk -v chr="$chr" '{print chr"\t"$1"\t"$2"\t"1}' > /net/bipolar/jedidiah/mutation/reference_data/GoNL_DNMs2_chr${chr}_mu.txt
