...
print $wFH "#SBATCH --exclude=inpsyght,psoriasis \n";
...
print $mdFH "#SBATCH --exclude=inpsyght,psoriasis \n";
...
$rawID=`squeue -u jedidiah --format \"%.24i %.9P %.24j %.8u %.2t %.10M %.6D\" | grep \"$jobcmd\" | awk '{print \$1}'`;
