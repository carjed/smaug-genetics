...
if (adj==1) {
	...
	ylabs<-unique(substr(names(pcm[-1]),1,8))
...
if (hot_flag=="on") {
	...
	hotm<-melt(hot, id=names(hot)[1:9])
	...
	ggplot(hotm2, aes(x=Centre, y=prop, colour=variable, width=End-Start))+
		...
		xlab("Position")+
		ylab("Relative Mutation Rate")
	suppressMessages(ggsave(recomb_out))
